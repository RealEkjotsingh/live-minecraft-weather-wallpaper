...
rainCanvas.style.zIndex = "5"; // below UI (9999), above video
...
function createDroplets(intensity) {
  droplets = [];

  let count = 0;

  if (intensity === 1)
    count = 40; // drizzle → more but tiny
  else if (intensity === 2) count = 60;
  else if (intensity === 3) count = 90;
  else count = 120;
  ...
  for (let i = 0; i < count; i++) {
    droplets.push({
      ...
      size: intensity === 1 ? 0.8 : Math.random() * 3 + 1, // tiny drizzle drops
      speed: intensity === 1 ? 0.15 : Math.random() * 0.5 + 0.2,
      opacity: intensity === 1 ? 0.15 : Math.random() * 0.3 + 0.2,
    });
  }
}
function animateDroplets() {
  ...
  ctx.filter = "blur(1px)"; // apply once (not per droplet)
  ...
  for (let d of droplets) {
    ...
    ctx.beginPath();
    ctx.ellipse(d.x, d.y, d.size * 0.6, d.size * 1.8, 0, 0, Math.PI * 2);
    ctx.fillStyle = `rgba(200,220,255,${d.opacity})`;
    ctx.fill();
  }

  requestAnimationFrame(animateDroplets); // ✅ OUTSIDE LOOP
}
...
function fetchWeather(lat, lon) {
  ...
  fetch(
    ...
    .then((r) => r.json())
    .then((data) => {
      if (requestMode !== "auto" && requestMode !== "fallback") return;
      // 🔴 CRITICAL CHECK
      if (!data || !data.current || !data.location) {
...
function renderWeather(resp) {
  ...
  if (rainIntensity > 0) {
    if (rainIntensity < 0.5)
      ...
      rainLevel = 2; // light
    else if (rainIntensity < 7)
      rainLevel = 3; // medium
    else rainLevel = 4; // heavy
  }
 ...
 const rainDisplay = (rainText || "").toUpperCase();
  ...
  if (weatherType === "Rain") {
    createDroplets(rainLevel);
  } else {
    droplets = [];
    ctx.clearRect(0, 0, rainCanvas.width, rainCanvas.height); // 🔥 add this
  }
...
${
  weatherType === "Snow"
    ...
    : rainIntensity > 0
      ...
      : // 🔥 keep text + add %
        ["RAIN POSSIBLE", "SHOWERS LIKELY", "STORM LIKELY"].includes(
            rainDisplay,
          )
        ? `· ${rainDisplay} · ${precipPercent}%`
        : rainDisplay
          ? `· ${rainDisplay}`
...
// 🔥 Safe initial load (use time logic instead)
updateWeatherAuto();
